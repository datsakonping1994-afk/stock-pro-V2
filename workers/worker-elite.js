...
const ELITE_PEOPLE_DEFAULT = [
  ...
  { id: 'warsh',    name: 'Kevin Warsh',         emoji: '🔴', role: 'Fed Chair' },
...
// [FIX item 2] ขยับเลขนี้ทุกครั้งที่แก้ไขข้อมูลใน ELITE_PEOPLE_DEFAULT (เช่น เปลี่ยนชื่อ/id คน)
// เพื่อให้ cache ใน KV (elite:people) ที่มี version ไม่ตรงถูกมองว่า "หมดอายุ" และรีเฟรชใหม่ทันที
// โดยไม่ต้องลบ KV key ด้วยมือ -- ป้องกันปัญหาข้อมูลเก่าค้างนานสุด 24 ชม. หลัง deploy
const ELITE_PEOPLE_VERSION = 2; // v2: cook=Tim Cook, fed chair id=warsh (แก้เมื่อ 18 มิ.ย. 2026)
...
async function getElitePeople(env) {
  if (env.ALERT_KV) {
    try {
      ...
      if (cached) {
        ...
        if (version === ELITE_PEOPLE_VERSION && Date.now() - ts < 24 * 3600 * 1000) {
          console.log('[Wiki] using cached elite people (v' + version + ')');
          return data;
        }
        console.log('[Wiki] cache stale or version mismatch (cached v' + version + ', current v' + ELITE_PEOPLE_VERSION + ') -- refreshing');
      }
    } catch (e) { console.warn('[Wiki] KV read error:', e.message); }
  }

  console.log('[Wiki] refreshing elite people from Wikipedia...');
  const updated = ELITE_PEOPLE_DEFAULT.map(p => ({ ...p })); // deep-enough clone: object ใหม่ทุกตัว ป้องกัน mutate ของเดิม
  ...
  if (env.ALERT_KV) {
    try {
      await env.ALERT_KV.put('elite:people', JSON.stringify({ data: updated, ts: Date.now(), version: ELITE_PEOPLE_VERSION }), { expirationTtl: 86400 });
...
// ── สร้าง query อัตโนมัติจากชื่อ + role ของแต่ละคน ──
...
async function runEliteScan(env) {
  ...
  if (env.ALERT_KV) {
    ...
    // [FIX item 3] TTL เดิม 90s อาจสั้นเกินไปถ้า BLS+ข่าว 20 คน(5 batch)+AI call รวมกันช้ากว่า 90 วิ
    // (worst case ทุก fallback ถูกเรียกจริงอาจรวมหลักนาที) ขยับเป็น 240s ให้มี margin กันไม่ให้ lock
    // หมดอายุก่อน scan เสร็จจริง ซึ่งจะทำให้ /trigger ซ้อนกันยิง scan สองรอบพร้อมกันได้
  ...
  try {
    ...
    // ── ดึงข่าวจาก ELITE_PEOPLE ทุกคน โดยใช้ชื่อ + role สร้าง query อัตโนมัติ ──
    let newsContext = '';
    let allArticles = [];
    ...
    // [DEBUG] นับจำนวนข่าวที่ดึงมาได้ทั้งหมดก่อน dedup/filter
    console.log('[DEBUG] articles fetched:', allArticles.length);

    allArticles = deduplicateNews(allArticles);

    // [DEBUG] นับจำนวนข่าวหลัง deduplicate
    console.log('[DEBUG] articles after dedup:', allArticles.length);
    ...
    allArticles = allArticles.filter(a => {
      ...
      // [FIX 2] ผ่อนการกรอง: ข่าวสำคัญของ CEO/ผู้นำหลายข่าวไม่มี keyword ที่ detectCatalyst() จับได้
      // ยอมให้ผ่านได้ถ้า catalyst ตรง หรือถ้า source คุณภาพสูงมาก (>=8) แม้ catalyst จะจับไม่ได้
      return (
        (a.sourceQuality || 4) >= 4 &&
        (
          catalystScore > 0 ||
...
3. ถ้าไม่มีข่าวจริง impact>=6 ให้ return []
...
id ต้องเป็นหนึ่งใน: trump, warsh, jensen, cook, musk, nadella, zuck, altman, xi, putin, lagarde, opec, pichai, bezos, dimon, buffett, yellen, iger, modi, khamenei
...
เฉพาะ impact>=6 ไม่เกิน 5 รายการ`;
    ...
    // [DEBUG] นับจำนวน item ที่ AI ส่งกลับมา
    console.log('[DEBUG] AI returned:', items?.length || 0);
    ...
    for (const item of cleanedItems) {
      ...
      // [FIX 4] ผ่อน hasIndividualStock: ข่าวมหภาค/การเมืองบางข่าวมีผลต่อตลาดจริง
      // แต่ไม่ได้โยงหุ้นรายตัวโดยตรง — ยกเว้นให้กลุ่มผู้นำ/องค์กรมหภาคที่กำหนดไว้
      if (
        !hasIndividualStock(item) &&
        !['trump','xi','putin','opec','lagarde','modi','khamenei'].includes(item.id)
      ) { _mon.deduped++; continue; }
    ...
    // [FIX 3] ลด impact threshold จาก >=7 เป็น >=6 เพื่อให้ได้ข่าวคุณภาพ 3-5 รายการต่อรอบ
    newItems = newItems.filter(item => (item.impact || 0) >= 6);
    if (!newItems.length) { LOG.info('no high-impact items'); return { ok: true, count: 0 }; }

    // [DEBUG] นับจำนวน item สุดท้ายก่อนบันทึก/ส่ง
    console.log('[DEBUG] final items:', newItems.length);

    // [FIX 5] ย้าย KV.put มาไว้หลัง filter ทั้งหมด เพื่อให้ข้อมูลใน KV, UI และ Telegram ตรงกัน
    if (env.ALERT_KV) {
      try {
